...
if __name__ == "__main__":
    ...
    print('Ploting ...')
    output_fig_fn = '01.AS_FGA_cutoff_response.pdf' # png
    ...
    for ct in cancerType_list:
        ...
        AS_elbowPts_list.append([kn_mean,kn_025,kn_975])
    ...
    for ct in cancerType_list:
        ...
        kn_025 = np.quantile(kn_bootstrap_ct, 0.025)
        kn_975 = np.quantile(kn_bootstrap_ct, 0.975)
        FGA_elbowPts_list.append([kn_mean, kn_025, kn_975])
        print('%50s: %.2f (%.2f, %.2f)' % (ct, kn_mean, kn_025, kn_975))

    plt.savefig(output_fig_fn) # , dpi=400
    plt.close()

    fnOut = open('CNAcutoff_elbow_result.txt', 'w')
    fnOut.write('CancerType\tEP_AS\tEP_AS_low\tEP_AS_up\tEP_FGA\tEP_FGA_low\tEP_FGA_up\n')
    for i in range(len(AS_elbowPts_list)):
        content = [cancerType_list[i]] + AS_elbowPts_list[i] + FGA_elbowPts_list[i]
